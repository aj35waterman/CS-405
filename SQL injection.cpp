...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    records.clear();
    //making lower case copy of the sql query to check for injection OR, or, and Or, etc.
    std::string localCopy(sql);
    std::transform(localCopy.begin(), localCopy.end(), localCopy.begin(), ::tolower);
    // looking for SQL injection pattern OR value=value

	std::regex pattern(R"(\bor\b\s+\S+\s*=\s*\S+)");//this regex looks for the pattern of "OR" followed by some value, an equals sign, and another value, which is a common 
    //pattern in SQL injection attacks. 
    //The \b ensures that we are matching "OR" as a whole word, and the \s* allows for any amount of whitespace around the equals sign.
    //check for the pattern in the lower case version of the SQL query to ensure we catch all variations of OR.
    if (std::regex_search(localCopy, pattern))// if the pattern is found, we have a potential SQL injection attack, so we log it and return false
    {
        std::cout << "SQL Injection pattern detected in query: " << std::endl << sql << std::endl; // log the suspicious query for further analysis
        return false; // return false to indicate the query should not be executed due to potential SQL injection
    }
